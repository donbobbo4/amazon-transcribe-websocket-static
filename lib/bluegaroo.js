...
var glossary = [
    {
        'term':'FACETS',
        'termPrinted':'Facets',
        'definition':'Membership management tool'
    },
    {
        'term':'CLAIM',
        'termPrinted':'Claim',
    ...
    },
    {
        'term':'ADVAIR',
        'termPrinted':'Advair',
        'definition':'Caution, this drug requires Prior Authorization.'
    }
  ];
    ...
    if (results.length > 0) {
      if (results[0].Alternatives.length > 0) {
        ...
        for(i=0; i < glossary.length; i++) {
            if (transcript.toUpperCase().indexOf(glossary[i].term) > 0) {
                foundTextArea = $('#found').val();
                if (foundTextArea.toUpperCase().indexOf(glossary[i].term) == -1) {
                    let newTerm = glossary[i].termPrinted + ': ' + glossary[i].definition;
                    $('#found').val(foundTextArea + newTerm + doubleReturn + divider + doubleReturn);
                    break;
                }
            }
        }
        //THIS WORKED.
        // if (transcript.toUpperCase().indexOf("FACETS") > 0) {
        //     $('#found').val(glossary[0].term + ': ' + glossary[0].definition);
        // }

          // if (glossary.term == transcript.toUpperCase) {
          //   $('#found').val(glossary.term + ': ' + glossary.definition);
          // }
...
function createPresignedUrl() {
    ...
    return v4.createPresignedURL(
        ...
        crypto.createHash('sha256').update('', 'utf8').digest('hex'), {
            'key': 'AKIARNRTMXGMLQ6GUYRY',
            'secret': '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',
            'sessionToken': '',
            'protocol': 'wss',
            'expires': 15,
            'region': region,
